...
function ReleasePage({ path, pageContext }) {
  ...
  const { title, year, genres, description, label, youtubeId, youtubeOptions, bandcampId, bandcampOptions } = pageContext.releaseData;
  ...
  return (
    <div className="virtualPhotography" id="galleryPage">
      ...
      {!!year && <h3 className="releasePage-year"><span className="releasePage-yearLabel">year = </span>{year}</h3>}
      {!!label && <h3 className="releasePage-label"><span className="releasePage-labelLabel">label = </span>{label}</h3>}
      ...
      {bandcampId && <BandcampEmbed albumId={bandcampId} size="large" options={{ width: '700px' }} />}
    </div>
    </div>
  );
}
